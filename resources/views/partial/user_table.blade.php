                            ...
                            @foreach ($data as $row)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route($page . '.show', $row->id) }}" title="show"
                                            class="btn btn-info btn-sm">
                                            <i class="fas fa-eye icon-show"></i>&nbsp;Show
                                        </a>
                                        <form style="display:inline-block" id="delete-form{{ $row->id }}"
                                            action="{{ route($page . '.destroy', $row->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="#"
                                                onclick="confirm('Are you sure, you want to delete {{ ucfirst($row->name) }} ?')?document.getElementById('delete-form{{ $row->id }}').submit():''"
